...
namespace EIP.Repository
{
    ...
    public class RepaymentPlanRepository : DefaultRepository<RepaymentPlan>
    {
        ...
        public List<RepaymentPlan> QueryRepaymentPlan(QueryModel model, out int totalCount)
        {
            ...
            string sql = "select * from dbo.RepaymentPlan where LogicDeleteFlag=0 and RepaymentPlantGUID like @p0 ";

            //分页查询必须要有排序字段
            model.SortField = string.IsNullOrEmpty(model.SortField) ? "RepaymentPlantGUID" : model.SortField;
